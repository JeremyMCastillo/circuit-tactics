...
func show_dialog():
	...
	fileDialog.get_vbox().set_custom_minimum_size(Vector2(500,500))
	...
	if not fileDialog.is_connected("file_selected",Callable(self,"on_file_selected")):
		fileDialog.connect("file_selected",Callable(self,"on_file_selected"))

func on_file_selected(_path):
	print("FILE SELECTED")
	on_files_selected([_path])
		...
		for _imagePath in imagesPath:
			...
			for _frame in range(_startFrame,_endFrame):
				...
				if !_root.has_node(_imageName):
					_newSpriteNode = AtlasTexture.new()
					_newSpriteNode.atlas = _newTexture
...
func getFileName(_path):
	var _fileName = _path.get_slice("/", _path.get_slice_count("/") - 1)
	_fileName = _fileName.get_slice(".", 0)
	return _fileName
